...
- packages
  ...
  - dolby
    ...
    - not upstream as no precedence for wrapping vapoursynth plugins with versioning(meson.build's are heavily modified)
  ...
  - vs
    - requires dependencies on python-numpy/python
      - not upstream as newer versions of plugins require newer vapoursynth or python versions for pypi packages
