(define-module (jam ci)
  ...
  #:use-module ((guix scripts pack)
                #:select (self-contained-tarball self-contained-appimage
                                                 docker-image guix-pack))
...
(define-public (cuirass-jobs store arguments)
  ...
  (define channels
    (let ((channels (assq-ref arguments
                              'channels)))
  ...
  (define source
    (channel-url guix))
  (parameterize ((current-guix-package (channel-source->package source
                                                                #:commit
                                                                commit)))
    (append-map (lambda (system)
                  ...
                  (list (->job "binary-relocatable-guile"
                                                       ...
                                                       (>>= (profile-derivation
                                                            ...
                                                            (lambda (profile)
                                                              (self-contained-appimage
                                                               ...
                                                               (list
                                                                     #:relocatable?
                                                                     #t) ;appimage require relocatable
                                                               #:entry-point
                                                               "bin/hello"))))
                                               #:system system))
                        ...
                        (image->job store
                                    (os->image %jam-vm
                                               #:type qcow2-image-type)
                                    #:name "binary-vm"
                                    #:system system) ;(image (inherit mbr-hybrid-disk-image) (operating-system %jam-vm)) ; usb image
                        (image->job store
