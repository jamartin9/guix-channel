       ...
       ((#:phases phases)
        (let ((python-version (version-major+minor (package-version python))))
          #~(modify-phases #$phases
              (add-after 'install 'pyinstall
                ;; copy libzfs_core python lib
                (lambda* (#:key inputs native-inputs make-flags
                          parallel-build? #:allow-other-keys)
                  (copy-recursively "contrib/pyzfs/build/lib"
                ...
                (lambda* (#:key inputs outputs #:allow-other-keys)
                  (let* ((out (assoc-ref outputs "out")))
                    ...
                    (invoke "./configure"
                            ...
                            (string-append "--with-pammoduledir=" out
                                           "/lib/security") ;pam
                            (string-append "--with-pamconfigsdir=" out
                                           "/etc/pam.d") ;pam
                            (string-append "--prefix=" out)
                            (string-append "--with-dracutdir=" out
                                           "/lib/dracut")
                            (string-append "--with-udevdir=" out "/lib/udev")
                            (string-append "--with-mounthelperdir=" out
                                           "/sbin")
