  ...
  (package
    ...
    (source
     (origin
       ...
       (sha256
        (base32 "08n4p56v9s58apw6nd9cwsmmyniwq2c2wh522aqljs34cbn1ir51"))))
    (arguments
                                ...
                                (add-after 'install 'really-install
                                  ...
                                  (lambda* (#:key outputs #:allow-other-keys)
                                    (let ((out (assoc-ref outputs "out")))
                                      ...
                                      (install-file "libpar2-turbo.a"
                                                    (string-append out "/lib"))
                                      (install-file "libgf16.a"
                                                    (string-append out "/lib"))
                                      (install-file "libhasher.a"
                                                    (string-append out "/lib")) ;add includes after configure for version.h
  ...
  (package
    ...
    (source
     (origin
       ...
       (file-name (git-file-name name version))
       (sha256
        (base32 "0caw4fda519a7hmmalc8zv0400dxzqlphy7y9ccag1a03nn0pbfl"))))
    (arguments
     (append (list #:configure-flags '(list "-DDISABLE_ENCODE=ON"
             ...
             (list #:phases #~(modify-phases %standard-phases
                                ...
                                (add-after 'install 'really-install
                                  (lambda* (#:key outputs #:allow-other-keys)
                                    (let ((out (assoc-ref outputs "out")))
                                      ;; (exit 1)
                                      (install-file "../source/rapidyenc.h"
                                                    (string-append out
                                                                   "/include"))
                                      (install-file
                                       "rapidyenc_static/librapidyenc.a"
                                       (string-append out "/lib")))))))))
    (outputs '("out"))
    (build-system cmake-build-system)
    (synopsis "C compatible library provides functions for implementing yenc")
    (description
     "C compatible library provides functions for implementing yenc")
