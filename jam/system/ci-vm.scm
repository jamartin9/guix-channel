...
"))
    ...
    (packages (append (list isc-dhcp nss-certs sudo cloud-utils) ;cloud-utils for growpart with resize2fs after img resize
                      %base-packages-utils %base-packages-linux)) ;wget man-db guix-icons; podman flatpak gvfs waypipe
    (services
     (append (list
              ...
              (service openssh-service-type
                       (openssh-configuration ;(%auto-start? #f)
                                              ...
                                              (authorized-keys `(("guest" ,(plain-file
                                                                            "jam.pub"
              ...
              (service nftables-service-type
                       (nftables-configuration (ruleset (plain-file
                                                         "nftables.conf"
                                                         "# Firewall hook order is: prerouting -> input/output/forward -> postrouting
                   # flush all the rules.
                   flush ruleset
                   # Define vars.
                   #define wan = eno1
